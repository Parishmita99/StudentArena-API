...
def recommend(jobs_user_likes):
    if jobs_user_likes not in df['jobtitle'].unique():
        ...
        recommendation_data = pd.DataFrame(columns=['Jobid','Jobtitle','JobDescription','Company'])

        recommendation_data['Jobid'] = job_id
        recommendation_data['Jobtitle'] = job_title
        recommendation_data['JobDescription'] = job_desc
